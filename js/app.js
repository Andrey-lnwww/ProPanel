(() => {
    ...
    (() => {
        ...
        let partnersSwiper = document.querySelector(".partners__slider");
        let partnersSwiperContainer = document.querySelector(".partners__container");
        let mySwiper;
        function initSliders() {
            if (document.querySelector(".examples__swiper")) new core(".examples__swiper", {
                ...
                on: {
                    ...
                    progress: function(swiper) {
                        for (let i = 0; i < swiper.slides.length; i++) {
                            let slideProgress = swiper.slides[i].progress, innerOffset = swiper.width * interleaveOffset, innerTranslate = slideProgress * innerOffset;
                            swiper.slides[i].querySelector(".examples-slide__figure").style.transform = "translateX(" + innerTranslate + "px)";
                        }
                    },
                    ...
                    setTransition: function(swiper, speed) {
                        for (let i = 0; i < swiper.slides.length; i++) {
                            ...
                            swiper.slides[i].querySelector(".examples-slide__figure").style.transition = speed + "ms";
                        }
                    }
                }
            });
            if (document.querySelector(".suppliers__slider")) new core(".suppliers__slider", {
                modules: [],
                slidesPerView: "auto",
                spaceBetween: 20,
                speed: 1e3,
        ...
        function mobileSlider() {
            if (partnersSwiper && window.innerWidth <= 600 && partnersSwiperContainer.dataset.mobile == "false") {
                mySwiper = new core(".partners__slider", {
                    modules: [ Autoplay ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: "3",
                ...
                const callback = (entries, observer) => {
                    entries.forEach((entry => {
                        if (window.innerWidth <= 600) if (entry.isIntersecting) mySwiper.autoplay.start(); else mySwiper.autoplay.stop(); else observer.disconnect();
                    }));
                };
                const observer = new IntersectionObserver(callback);
                observer.observe(partnersSwiperContainer);
                partnersSwiperContainer.dataset.mobile = "true";
            }
            if (window.innerWidth > 600) {
                partnersSwiperContainer.dataset.mobile = "false";
                if (partnersSwiper.classList.contains("swiper-initialized")) mySwiper.destroy();
            }
        }
        mobileSlider();
